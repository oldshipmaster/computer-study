...
const roots = [new URL("../components/lessons/", import.meta.url), new URL("../lib/", import.meta.url)];
...
test("uses only reserved example domains inside lesson simulations", () => {
  const urls = roots.flatMap(sourceFiles).flatMap((file) => [...readFileSync(file, "utf8").matchAll(/https?:\/\/([a-z0-9.-]+)/gi)].map((match) => [file.pathname, match[1]]));
