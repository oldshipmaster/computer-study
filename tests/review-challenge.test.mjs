...
test("covers every island with two scenario questions", () => {
  assert.equal(REVIEW_QUESTIONS.length, 12);
  const counts = REVIEW_QUESTIONS.reduce((result, question) => {
    result[question.islandId] = (result[question.islandId] ?? 0) + 1;
    return result;
  }, {});
  assert.deepEqual(Object.values(counts), [2, 2, 2, 2, 2, 2]);
  assert.equal(new Set(REVIEW_QUESTIONS.map((question) => question.id)).size, 12);
});
...
test("answering after completion is safe and deterministic", () => {
  const state = { ...createReviewState(), index: 11, score: 12, completed: true };
