...
test("server-renders the complete curriculum map", async () => {
  ...
  assert.equal((html.match(/course-card--available/g) ?? []).length, 8);
  assert.equal((html.match(/course-card--locked/g) ?? []).length, 12);
  assert.equal((html.match(/disabled=""/g) ?? []).length, 12);
