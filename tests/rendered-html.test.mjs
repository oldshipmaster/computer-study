...
test("server-renders the complete curriculum map", async () => {
  ...
  assert.equal((html.match(/course-card--available/g) ?? []).length, 5);
  assert.equal((html.match(/course-card--locked/g) ?? []).length, 15);
  assert.equal((html.match(/disabled=""/g) ?? []).length, 15);
