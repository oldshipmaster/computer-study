...
export interface BackupProgress { version: 1; completedCourseIds: string[]; badgeIds: string[]; settings: { sound: boolean; reducedMotion: boolean }; resume: { courseId: string; stage: number } | null; }
...
export function parseProgressBackup(text: string): BackupParseResult {
  try {
    ...
    return { ok: true, progress: { ...parsed, completedCourseIds, badgeIds: parsed.badgeIds.filter((id) => /^[a-z0-9-]{1,64}$/i.test(id)), resume } };
  } catch {
