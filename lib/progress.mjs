import { getIslandBoss, getIslandBossUnlock, sanitizeCompletedBossIds } from "./island-boss.ts";
 ...
 * @property {string[]} completedBossIds
...
export function parseProgress(raw) {
  ...
  try {
    ...
    return {
      ...
      completedBossIds: sanitizeCompletedBossIds(stored.completedBossIds, completedCourseIds),
 ...
 * @returns {ProgressState}
 */
export function completeIslandBoss(progress, bossId) {
  const boss = getIslandBoss(bossId);
  if (!boss || !getIslandBossUnlock(boss, progress.completedCourseIds).unlocked || progress.completedBossIds?.includes(bossId)) {
    return progress;
  }
  return {
    ...progress,
    completedBossIds: sanitizeCompletedBossIds([...(progress.completedBossIds ?? []), bossId], progress.completedCourseIds),
  };
}
