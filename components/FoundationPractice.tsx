...
export function FoundationPractice({ onStartCourse }: { onStartCourse: (courseId: string) => void }) {
  ...
  function answer(option: string, detail: number) {
    if (correct || isRepeatedPointerActivation(detail)) return;
    setChoice(option);
    if (option === question.answer) setScore((value) => value + 1);
  }

  useLayoutEffect(() => { if (movingRef.current) { movingRef.current = false; questionHeadingRef.current?.focus(); } }, [index, round]);
  function next(detail: number) { if (isRepeatedPointerActivation(detail)) return; movingRef.current = true; setIndex((value) => value + 1); setChoice(null); }
  function restart(detail: number) { if (isRepeatedPointerActivation(detail)) return; movingRef.current = true; setRound((value) => value + 1); setIndex(0); setChoice(null); setScore(0); }
  ...
  return <section className="foundation-practice" id="foundation-practice" aria-labelledby="foundation-practice-heading">
    ...
    <div className="foundation-practice-console">
      ...
      <div role="group" aria-label="选择答案">{question.options.map((option) => <button aria-pressed={choice === option} disabled={correct} key={option} onClick={(event) => answer(option, event.detail)} type="button">{option}</button>)}</div>
      ...
      <div className="foundation-practice-actions"><button onClick={() => onStartCourse(question.courseId)} type="button">回到对应课程实验</button>{correct && !finished ? <button onClick={(event) => next(event.detail)} type="button">下一题</button> : null}{finished ? <button onClick={(event) => restart(event.detail)} type="button">再来一组 · 本轮 {score}/4</button> : null}</div>
    </div>
  </section>;
}
