...
export function PacketEscort({ completedCourseIds, onStartCourse }: PacketEscortProps) {
  ...
  return (
    <section className="packet-escort-shell packet-escort-game" id="packet-escort" aria-labelledby="packet-escort-game-heading">
      ...
      {game.phase === "pack" ? (
        <section className="packet-stage" aria-labelledby="packet-pack-heading">
          ...
          <div className="packet-pack-layout">
            ...
            <div className="packet-layer-actions" role="group" aria-label="选择下一层">{PACK_LAYER_ORDER.map((layer) => <button className="packet-escort-action" disabled={game.packedLayers.includes(layer)} key={layer} onClick={(event) => setGame((current) => packPacketLayer(current, mission, layer, event.detail))} type="button"><b>{LAYER_COPY[layer].icon}</b><span>{LAYER_COPY[layer].label}</span><small>{LAYER_COPY[layer].purpose}</small></button>)}</div>
          </div>
        </section>
      ) : null}
      ...
      {game.phase === "route" ? (
        <section className="packet-stage" aria-labelledby="packet-route-heading">
          ...
          <div className="packet-route-grid">{mission.routes.map((route) => <button className="packet-route-card packet-escort-action" key={route.id} onClick={(event) => setGame((current) => chooseEscortRoute(current, mission, route.id, event.detail))} type="button"><strong>{route.label}</strong><span>{route.nodes.join(" → ")}</span><b>总代价 {route.cost}</b></button>)}</div>
        </section>
      ) : null}
      ...
      {game.phase === "send" ? (
        <section className="packet-stage" aria-labelledby="packet-send-heading">
          ...
          <button className="packet-escort-action packet-primary-action" onClick={(event) => setGame((current) => launchPacketTransfer(current, mission, event.detail))} type="button">发射全部编号分组</button>
        </section>
      ) : null}
      ...
      {game.phase === "retransmit" ? (
        <section className="packet-stage" aria-labelledby="packet-retransmit-heading">
          ...
          <div className="packet-arrival-grid" role="group" aria-label="分组到达证据">{mission.fragments.map((fragment, sequence) => { const arrived = game.receivedOrder.includes(sequence); return <button aria-label={`第 ${sequence + 1} 组，${arrived ? "已到达" : "缺失，点击重传"}`} className={`packet-escort-action ${arrived ? "is-arrived" : "is-missing"}`} disabled={arrived} key={sequence} onClick={(event) => setGame((current) => retransmitPacket(current, mission, sequence, event.detail))} type="button"><b>#{sequence + 1}</b><span>{fragment}</span><small>{arrived ? "已确认" : "没有确认"}</small></button>; })}</div>
        </section>
      ) : null}
      ...
      {game.phase === "order" ? (
        <section className="packet-stage" aria-labelledby="packet-order-heading">
          ...
          <button className="packet-escort-action packet-primary-action" onClick={(event) => setGame((current) => confirmPacketOrder(current, mission, event.detail))} type="button">确认编号并组装消息</button>
        </section>
      ) : null}
      ...
      {game.phase === "unpack" ? (
        <section className="packet-stage" aria-labelledby="packet-unpack-heading">
          ...
          <div className="packet-unpack-layout"><div className="packet-envelope-stack is-receiving" aria-label="等待拆除的网络层" role="img"><span className="packet-message-core">恢复的消息</span>{PACK_LAYER_ORDER.map((layer, index) => <span className={`packet-envelope layer-${layer} ${game.unpackedLayers.includes(layer) ? "is-unpacked" : "is-packed"}`} key={layer} style={{ inset: `${26 - index * 7}px` }}><b>{LAYER_COPY[layer].label}</b></span>)}</div><div className="packet-layer-actions" role="group" aria-label="选择要拆除的层">{UNPACK_LAYER_ORDER.map((layer) => <button className="packet-escort-action" disabled={game.unpackedLayers.includes(layer)} key={layer} onClick={(event) => setGame((current) => unpackPacketLayer(current, mission, layer, event.detail))} type="button"><b>{LAYER_COPY[layer].icon}</b><span>拆 {LAYER_COPY[layer].label}</span></button>)}</div></div>
        </section>
      ) : null}

      {game.phase === "solved" ? (
        <section className="packet-stage packet-message-recovered" aria-labelledby="packet-recovered-heading"><span aria-hidden="true">✓</span><div><p>恢复的消息</p><h3 id="packet-recovered-heading">{game.assembledMessage}</h3><strong>四层全部正确移除，消息内容没有丢字也没有乱序。</strong></div><button className="packet-escort-action" onClick={nextMission} type="button">{game.missionIndex === game.missionCount - 1 ? "查看护航报告" : "接入下一次护航"} →</button></section>
      ) : null}
