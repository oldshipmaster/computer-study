...
export function TermMatchChallenge({ completedCourseIds }: { completedCourseIds: string[] }) {
  ...
  return <section className="term-match" id="term-match" aria-labelledby="term-match-heading">
    ...
    <div className="term-match-console">
      ...
      {!question ? <div className="term-match-locked"><span aria-hidden="true">🔒</span><p>先完成任意一课，就会解锁对应的核心词。</p></div> : state.completed ? <div className="term-match-finish"><span aria-hidden="true">🎯</span><h3>概念全部配对成功</h3><p>试着不用看答案，把其中一个词讲给家长听。</p><button onClick={restart} type="button">再配对一次</button></div> : <div className="term-match-question"><span>第 {state.index + 1} / {questions.length} 题</span><h3>{question.prompt}</h3><div>{question.options.map((option, index) => <button key={option} onClick={() => setState((current) => answerTermMatch(current, index, questions))} type="button">{option}</button>)}</div><p role="status">{state.feedback}</p></div>}
    </div>
  </section>;
}
