...
export function FoundationCapstone({ completedCourseIds }: { completedCourseIds: readonly string[] }) {
  ...
  function next(detail: number) { if (isRepeatedPointerActivation(detail)) return; movingRef.current = true; if (index === FOUNDATION_CAPSTONE.length - 1) setFinished(true); else { setIndex((value) => value + 1); setChoice(null); } }
  function restart(detail: number) { if (isRepeatedPointerActivation(detail)) return; movingRef.current = true; setIndex(0); setChoice(null); setFinished(false); }
  ...
  return <section className="foundation-capstone" id="foundation-capstone" aria-labelledby="foundation-capstone-heading">
    ...
    <div className="foundation-capstone-console">
      {finished ? <div className="foundation-capstone-finish"><span aria-hidden="true">🧠</span><h3 ref={finishHeadingRef} tabIndex={-1}>五条系统航线全部接通</h3><p>你已经能把不同领域的知识放进同一个问题里思考。</p><button onClick={(event) => restart(event.detail)} type="button">重新挑战</button></div> : <><p>{mission.domains.join(" × ")} · {index + 1}/5</p><h3 ref={questionHeadingRef} tabIndex={-1}>{mission.prompt}</h3><div role="group" aria-label="选择系统方案">{mission.options.map((option) => <button aria-pressed={choice === option} disabled={correct} key={option} onClick={(event) => answer(option, event.detail)} type="button">{option}</button>)}</div><p className={correct ? "is-correct" : ""} role="status">{choice ? correct ? `方案成立：${mission.explanation}` : `还缺一环：${mission.explanation}` : "先找出题目连接了哪些领域，再选择方案。"}</p>{correct ? <button className="primary-action" onClick={(event) => next(event.detail)} type="button">{index === 4 ? "接通总控台" : "下一条系统航线"}</button> : null}</>}
    </div>
  </section>;
}
