...
export function DeviceCoordinationLab({ onComplete }: { onComplete: () => void }) {
  ...
  return <div className="advanced-lab device-coordination-lab"><h2>设备协调中心挑战</h2><p>操作系统通过对应驱动，按顺序处理程序的设备请求。</p><div role="group" aria-label="选择设备队首请求">{pending.map((request, index) => <button key={request.id} onClick={(event) => service(request, event.detail)} type="button"><small>队列第 {index + 1} 位</small>{request.task} → {request.device}</button>)}</div><p role="status">{feedback} 已服务：{serviced.length ? serviced.map((request) => request.task).join("、") : "还没有"}；等待：{pending.length} 项</p>{pending.length ? null : <button onClick={onComplete} type="button">完成设备协调</button>}</div>;
}
