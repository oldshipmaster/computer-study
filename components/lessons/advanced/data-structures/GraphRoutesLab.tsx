...
export function GraphRoutesLab({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    <div className="advanced-lab graph-routes-lab">
      ...
      <div className="graph-node-map" role="group">{Object.keys(GRAPH).map((id) => <button aria-pressed={path.includes(id)} disabled={id !== current && (!GRAPH[current].includes(id) || path.includes(id))} key={id} onClick={() => travel(id)} type="button">{LABELS[id]}</button>)}</div>
