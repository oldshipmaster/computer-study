...
export function PromptInspector({ onComplete }: Props) { const [index, setIndex] = useState(0); const [feedback, setFeedback] = useState("先检查来源和请求，再选择行动。" ); const current = CASES[index]; const inspection = inspectPrompt(current.signals); function choose(action: PromptAction) { if (action !== inspection.action) { setFeedback("先别点弹窗里的内容。检查来源、催促、秘密和下载线索。" ); return; } if (index === CASES.length - 1) onComplete(); else { setIndex((value) => value + 1); setFeedback("判断完成，迷雾散开一层。" ); } } return <div className="prompt-inspector"><div className="fake-browser"><header><span>虚拟页面 · 不会打开链接</span><code>{current.source}</code></header><section><h2>{current.title}</h2><p>{current.body}</p><small>观察到的线索：{inspection.warningFlags.length ? inspection.warningFlags.join("、") : "来源可信、内容符合预期"}</small></section></div><div className="prompt-actions">{ACTIONS.map((action) => <button key={action.id} onClick={() => choose(action.id)} type="button">{action.label}</button>)}</div><p role="status">{feedback} 案例 {index + 1}/5</p></div>; }
