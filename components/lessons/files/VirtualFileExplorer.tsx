...
export function VirtualFileExplorer({ state, onAction }: Props) {
  ...
  return <section className="virtual-explorer" aria-label="虚拟文件浏览器">
    <header><button disabled={state.path.length === 0} onClick={() => onAction({ type: "goBack" })} type="button">← 上一级</button><button onClick={() => onAction({ type: "goRoot" })} type="button">⌂ 根目录</button><div className="file-address" aria-label={`当前位置 ${state.address}`}>{state.address}</div></header>
    ...
    <div className="file-keyboard-actions" aria-label="键盘操作">{entries.map((entry) => <button key={entry.name} onClick={() => onAction({ type: entry.kind === "folder" ? "openFolder" : "openFile", name: entry.name })} type="button">打开{entry.name}</button>)}</div>
