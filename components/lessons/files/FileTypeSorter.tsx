...
export function FileTypeSorter({ onComplete }: Props) {
  ...
  return <div className="file-type-sorter"><div className="unsorted-files" aria-label="待分类文件" role="group">{FILES.map((name) => <button aria-pressed={selected === name} disabled={Boolean(state.sorted[name])} key={name} onClick={() => setSelected(name)} type="button">{state.sorted[name] ? "✓ " : ""}{name}</button>)}</div><div className="type-trays">{TRAYS.map((tray) => <button key={tray.id} onClick={() => place(tray.id)} type="button"><span>{tray.icon}</span><strong>{tray.label}</strong><small>{Object.values(state.sorted).filter((value) => value === tray.id).length} 个</small></button>)}</div><p role="status">{state.wrongAttempts ? "进度还在。看看文件名最后的扩展名，再选一次。" : "先选文件，再选分类盒。"}</p><aside>陌生文件即使改了扩展名也不一定安全，不打开，先问可信的大人。</aside></div>;
}
